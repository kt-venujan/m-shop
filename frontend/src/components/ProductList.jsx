          ...
          <div className="bg-white grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 divide-x divide-y md:divide-y-0 divide-gray-100 shadow-sm border-b border-gray-100">
            {flashProducts.map(product => (
              <Link to={`/product/${product._id}`} key={product._id} className="p-3 hover:shadow-[0_0_10px_rgba(0,0,0,0.1)] transition-shadow group relative block bg-white h-full flex flex-col justify-start">
                <div className="aspect-square mb-2 relative overflow-hidden bg-white">
                  {product.image ? (
          ...
          <div className="bg-white shadow-sm border border-gray-100">
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">
              {uniqueCategories.map((category, idx) => {
                ...
                return (
                  <Link to={`/?category=${encodeURIComponent(category)}`} key={idx} className="flex flex-col items-center justify-center p-3 hover:shadow-[0_0_10px_rgba(0,0,0,0.1)] transition-shadow group relative bg-white h-[135px] text-center border-b border-r border-gray-100">
                    <div className="w-[70px] h-[70px] mb-2 object-contain overflow-hidden bg-white p-1 relative z-10 group-hover:scale-110 transition-transform">
                       {catProduct && catProduct.image ? (
                         <img src={catProduct.image} alt={category} className="w-full h-full object-contain mix-blend-multiply drop-shadow-sm" />
                       ) : (
                         <div className="w-full h-full bg-gray-50 rounded flex items-center justify-center text-2xl">📁</div>
                       )}
                    </div>
                    <span className="text-[12px] text-[#212121] leading-[14px] group-hover:text-orange-500 transition-colors line-clamp-2">{category}</span>
                  </Link>
                )
              })}
            </div>
